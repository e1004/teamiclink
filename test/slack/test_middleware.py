...
def test_it_adds_goal_store_to_ctx_and_calls_next(target: Target):
    # given
    context = BoltContext()
    next_func = MagicMock()

    # when
    target.slack_middleware.ctx_goal_store(context=context, next=next_func)

    # then
    assert context[SlackMiddleware.GOAL_STORE_KEY] == target.slack_middleware.GOAL_STORE
    next_func.assert_called_once_with()
